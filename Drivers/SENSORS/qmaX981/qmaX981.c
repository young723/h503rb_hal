...
qs32 qmaX981_writereg(qu8 reg_add, qu8 reg_dat)
{
	...
	while((!ret) && (retry++ < 5))
	{
		ret = bsp_i2c_write_reg(g_qmaX981.slave, reg_add, reg_dat);
	}
...
qs32 qmaX981_readreg(qu8 reg_add, qu8 *buf, qu16 num)
{
	...
	while((!ret) && (retry++ < 5))
	{
		ret = bsp_i2c_read_reg(g_qmaX981.slave, reg_add, buf, num);
	}
...
#if defined(QMAX981_HAND_RAISE_DOWN)
	qmaX981_hand_raise_down(1, QMAX981_MAP_INT1, QMAX981_ENABLE);
#endif
...
qs8 qmaX981_init(void)
{
	...
	for(index=0; index<2; index++)
	{
		...
		{			
			QMAX981_LOG("qmaX981 find slave=0x%x\n",g_qmaX981.slave);
			break;
		}
	}
	if(g_qmaX981.chip_id == QMA7981_DEVICE_ID)
