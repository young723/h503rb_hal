...
s8 qmp6989_burst_read(u8 u8Addr, u8* pu8Data, u8 u8Len)
{
	...
	if(ret >= 1)
		...
		comRslt = 0;
#else
	ret = bsp_i2c_read_reg(qmp6989_slave, u8Addr, pu8Data, u8Len);
	if(ret != 1)
		comRslt = -1;
	else
		comRslt = 1;
...
s8 qmp6989_burst_write(u8 u8Addr, u8* pu8Data, u8 u8Len)
{
	...
	for(i=0; i<u8Len; i++)
	{
		ret = bsp_i2c_write_reg(qmp6989_slave, u8Addr+i, pu8Data[i]);
	}
	if(ret != 1)
		comRslt = -1;
	else
...
s8 qmp6989_get_uuid(void)
{
	...
	QMP6989_DEBUG("qmp6989_get_uuid: 0x%x \n", qmp6989_uuid);

	//é»˜è®¤?å?ï¼Œä??¤æ–­è¿”å?ï¿?
	return comRslt;
}

s8 qmp6989_dump_reg(void)
{
	s8 comRslt = -1;
	u8 u8Data[4];
	
	comRslt = qmp6989_burst_read(QMP6989_REG_CONFIG2, u8Data, 3);
	QMP6989_DEBUG("qmp6989_dump_reg 0xa6~0xa8: [0x%x 0x%x 0x%x]\n", u8Data[0],u8Data[1],u8Data[2]);	
	comRslt = qmp6989_burst_read(QMP6989_REG_CALIB00, u8Data, 4);
	QMP6989_DEBUG("qmp6989_dump_reg 0xaa~0xad: [0x%x 0x%x 0x%x 0x%x]\n", u8Data[0],u8Data[1],u8Data[2],u8Data[3]);	
...
s8 qmp6989_get_calibration_param(float* fCalibParam){
  ...
  for(i = 0; i < QMP6989_CALIBRATION_PARAMETER_COUNT; ++i){
    ...
    fCalibParam[i] = ((tmp << shift) >> (shift + 2)) * QMP6989_POWER_SCALE[(u8DataBuf[2 * i + 1] & 0x03)] * QMP6989_CALIB_SCALE_FACTOR[i];
  	QMP6989_DEBUG("u8DataBuf[%d]=0x%x u8DataBuf[%d]=0x%x fCalibParam[%d]=%f \r\n", 2*i, u8DataBuf[2*i], 2*i+1, u8DataBuf[2*i+1], i, fCalibParam[i]);
  }
...
s8 qmp6989_measure_T(s16* ps16T){
  ...
  *ps16T = (s16)(((s16)u8Data[0] << 8) + u8Data[1]);
...
s8 qmp6989_measure_P(s32* ps32P){
  ...
  if(QMP6989_GET_BITSLICE(u8Data[0], QMP6989_DRDY) != 1)
  {
    QMP6989_DEBUG("status fail 0x%02x\n", u8Data[0]);
  }
  ...
  *ps32P = (s32)(((s32)u8Data[0] << 16) + ((s32)u8Data[1] << 8) + u8Data[2]);
