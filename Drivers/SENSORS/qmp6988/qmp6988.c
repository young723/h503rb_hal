...
uint8_t qmp6988_WriteReg(uint8_t slave, uint8_t reg_add,uint8_t reg_dat)
{
	...
	while((!ret) && (retry++ < 5))
	{
#ifdef QMP6988_USE_SPI
		ret = bsp_spi_write_reg(reg_add, reg_dat);
#else
		ret = bsp_i2c_write_reg(slave, reg_add, reg_dat);
#endif
	}
...
uint8_t qmp6988_ReadData(uint8_t slave, uint8_t reg_add,unsigned char* Read, uint16_t num)
{
	uint8_t ret = 0;
	uint32_t retry = 0;
	...
	{
#ifdef QMP6988_USE_SPI
		ret = bsp_spi_read_reg(reg_add, Read, num);
#else
		ret = bsp_i2c_read_reg(slave, reg_add, Read, num);
#endif
	}
...
uint8_t qmp6988_device_check(qmp6988_data *qmp6988)
{
	...
	qmp6988_ReadData(qmp6988->slave, QMP6988_CHIP_ID_REG, &chip_id, 1);
	if(chip_id == QMP6988_CHIP_ID)
	{
		QMP6988_LOG("qmp6988 slave:0x%x read chip id = 0x%x\n",qmp6988->slave, chip_id);
		return 1;
	}
	else
	{
		QMP6988_LOG("qmp6988 slave:0x%x read fail chip id = 0x%x !\n",qmp6988->slave, chip_id);
		return 0;
	}
}
...
#if (QMP6988_USE_SPI == 3)
	qmp6988_WriteReg(qmp6988->slave, QMP6988_IOSETUP_REG, 0x01);
#endif
	qmp6988_delay(20);
}
...
void qmp6988_set_app(qmp6988_data *qmp6988, unsigned char app)
{
	...
	if(app == 1)	//Weather monitoring, High speed, 0.05Hz, 1.2ua
	{
	...
	else if(app == 2)		//Drop detection, Low power, 100Hz, 407ua
	{
		qmp6988_set_oversampling_p(qmp6988, QMP6988_OVERSAMPLING_2X);
		qmp6988_set_oversampling_t(qmp6988, QMP6988_OVERSAMPLING_1X);
		qmp6988_set_filter(qmp6988, QMP6988_FILTERCOEFF_OFF);
	}
	else if(app == 3)		//Elevator detection, Standard, 10Hz 63.4ua
	{
		qmp6988_set_oversampling_p(qmp6988, QMP6988_OVERSAMPLING_8X);
		qmp6988_set_oversampling_t(qmp6988, QMP6988_OVERSAMPLING_1X);
		qmp6988_set_filter(qmp6988, QMP6988_FILTERCOEFF_4);
	}
	else if(app == 4)		//Stair detection, High Accuracy, 20Hz, 219ua
	{
	...
	else if(app == 5)		//Indoor navigation, Ultra High Accuracy, 28Hz, 570ua
	{
	...
	else if(app == 6)		//Indoor navigation, Ultra High Accuracy
	{
		qmp6988_set_oversampling_p(qmp6988, QMP6988_OVERSAMPLING_32X);
		qmp6988_set_oversampling_t(qmp6988, QMP6988_OVERSAMPLING_8X);
		qmp6988_set_filter(qmp6988, QMP6988_FILTERCOEFF_32);
	}
	else
	{	
		qmp6988_set_oversampling_p(qmp6988, QMP6988_OVERSAMPLING_1X);
		qmp6988_set_oversampling_t(qmp6988, QMP6988_OVERSAMPLING_1X);
		qmp6988_set_filter(qmp6988, QMP6988_FILTERCOEFF_OFF);
	}
}
...
void qmp6988_calc_pressure(qmp6988_data *qmp6988, float *press, float *tempearture)
{
	...
	{
		err = qmp6988_ReadData(qmp6988->slave, QMP6988_DEVICE_STAT_REG, a_data_u8r, 1);
		if(err == 0)
		{
			a_data_u8r[0] = 0x00;
		}
//		QMP6988_LOG("qmp6988 read status err=%d 0xf3 = 0x%02x \n", err, a_data_u8r[0]);
		qmp6988_delay(1);
		if(retry_count++ > 5)
		{
			qmp6988->fail_num++;
			if(qmp6988->fail_num > 5)
			{
				QMP6988_LOG("qmp6988_calc_pressure err re-init!!!\n");
				qmp6988->fail_num = 0;
				qmp6988_set_powermode(qmp6988, QMP6988_NORMAL_MODE);
				qmp6988_set_app(qmp6988, 4);
			}
			return;
		}
	}
	...
	//temp = (float)(T_int >> 8);
	temp = (float)T_int/256.0f;
	//pre = (float)(P_int >> 4);
	pre = (float)P_int/16.0f;
#else
	a0 = qmp6988->fk.a0;
	b00 = qmp6988->fk.b00;
	a1 = qmp6988->fk.a1;
...
void qmp6988_get_data(float *press, float *tempearture)
{
	qmp6988_calc_pressure(&d6988, press, tempearture);
}

int qmp6988_init()		/*qmp6988_data *qmp6988*/	
{
	uint8_t ret;
	
	d6988.slave = QMP6988_SLAVE_ADDRESS_L;
	ret = qmp6988_device_check(&d6988);
	if(ret == 0)
	{	
		d6988.slave = QMP6988_SLAVE_ADDRESS_H;
		ret = qmp6988_device_check(&d6988);
		if(ret == 0)
		{
			return 0;
	...
	qmp6988_software_reset(&d6988);
	qmp6988_delay(50);
#if (QMP6988_USE_SPI == 3)
	qmp6988_WriteReg(d6988.slave, QMP6988_IOSETUP_REG, 0x01);
#else
	qmp6988_WriteReg(d6988.slave, QMP6988_IOSETUP_REG, 0x00);
#endif
	qmp6988_delay(5);

	qmp6988_get_calibration_data(&d6988);
	qmp6988_set_powermode(&d6988, QMP6988_NORMAL_MODE);
	qmp6988_set_app(&d6988, 3);
	qmp6988_dumpreg(&d6988);
